...
#import "EDABbsHomeTableView.h"
#import "MFAllowGestureEventPassTableView.h"

@interface EDABbsHomeTableView () <UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate>
...
@implementation EDABbsHomeTableView
...
/// 滚动到顶或底
/// @param topType 1 顶  2 中间 3底
- (void)scrollToTopWithTopType:(NSInteger)topType {
    if (topType == 1) {
        self.scrolTotop = YES;
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(nestTableViewContentScrollToTopWithTopType:)]) {
        [self.delegate nestTableViewContentScrollToTopWithTopType:topType];
    }
}
...
- (CGFloat)heightForContainerCanScroll {
    ...
    if (_tableView && _tableView.tableHeaderView) {
        return CGRectGetHeight(_tableView.tableHeaderView.frame) - [self contentInsetTop] + 44;
    } else {
        return 0;
    }
}
...
- (void)resizeContentHeight {
    ...
    CGFloat contentHeight = CGRectGetHeight(self.bounds) - [self segmentViewHeight] - [self contentInsetTop] - [self contentInsetBottom] - [self footerViewHeight] + 44;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat insetTop = [self contentInsetTop];
    if (scrollView.contentOffset.y <= -insetTop+44 && _canScroll) {
        scrollView.contentOffset = CGPointMake(0, -insetTop+44);
        [self scrollToTopWithTopType:3];
    } else if (scrollView.contentOffset.y >= contentOffset){
        scrollView.contentOffset = CGPointMake(0, contentOffset);
        self.canScroll = NO;
        [self scrollToTopWithTopType:1];
    } else if (!_canScroll) {
        scrollView.contentOffset = CGPointMake(0, contentOffset);
        [self scrollToTopWithTopType:1];
    } else {
        [self scrollToTopWithTopType:2];
    }
