...
@property (nonatomic, strong) EDABbsHomeTableView *nestTableView;
...
- (void)initLayout {
    ...
    _nestTableView = [[EDABbsHomeTableView alloc] initWithFrame:self.view.bounds];
...
- (void)initHeaderView {
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
    imageView.frame = CGRectMake(0, -offsetTop+44, CGRectGetWidth(self.view.frame), self.view.frame.size.width * image.size.height / image.size.width);
...
- (void)initSegmentView {
    ...
    _segmentView = [[MFSegmentView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), 44)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    } else if (_nestTableView.scrolTotop) {
        if (setY<=0) {
            scrollView.contentOffset = CGPointZero;
    ...
    } else if (_nestTableView.scrolToBottom) {
        if (setY>=0) {
            scrollView.contentOffset = CGPointZero;
            _nestTableView.canScroll = YES;
            _canContentScroll = NO;
